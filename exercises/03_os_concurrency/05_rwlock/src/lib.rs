...
impl<T> RwLock<T> {
    ...
    pub fn write(&self) -> RwLockWriteGuard<'_, T> {
        ...
        loop {
            ...
            // At this point, no readers and no writer holding
            // We have WRITER_WAITING set, try to acquire
            match self.state.compare_exchange(s, WRITER_HOLDING | WRITER_WAITING, Ordering::AcqRel, Ordering::Acquire) {
